...
def main():
    ...
    lo_mags = np.array( zip(lo_z_mag_cosmos['g'],
                            lo_z_mag_cosmos['r'],
                            lo_z_mag_cosmos['i'],
                            lo_z_mag_cosmos['z']) )
    hi_mags = np.array( zip(hi_z_mag_cosmos['g'],
                            hi_z_mag_cosmos['r'],
                            hi_z_mag_cosmos['i'],
                            hi_z_mag_cosmos['z']) )
    ...
    print "Time to open files and set up tables: ", setup_time

    start = time.time()

    pool = Pool(processes=os.environ[''])

    results = pool.map(n, 
    n_lo_z_mag = n(mags, magerrs, lo_mags)
    n_hi_z_mag = n(mags, magerrs, hi_mags)
    n_lo_z_flux = n(mags, magerrs, hi_mags)
    n_hi_z_flux = n(mags, magerrs, hi_mags)

    work_time = time.time() - start
    print "Time to find densities: ", work_time
