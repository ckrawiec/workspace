...
def likelihoods(val, err, truevals):
    """
    returns likelihood from each trueval given val & err
    """
...
class Targets:
    ...
    def debug(self, templates, ntest, zranges, numthreads, output):
        ...
        col_defs = [fits.Column(name='template_z', format='D', array=template_zs),
                    fits.Column(name='template_id', format='A', array=template_ids)]
        ...
        id_col = fits.Column(name=self.id, format='A', array=debug_ids)
