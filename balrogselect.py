...
for table in tables:
    ...
    for table_num in table_nums:
        if table=='SIM':
            out_file = data_dir+'balrog_sva1_tab{}_SIM_flux_radii.fits'.format(table_num)
        elif table=='TRUTH':
            out_file = data_dir+'balrog_sva1_tab{}_TRUTH_fluxes.fits'.format(table_num)
        ...
        for band in bands:            
            ...
            if table=='SIM':
                query = "select BALROG_INDEX, ALPHAMODEL_J2000, DELTAMODEL_J2000, FLUX_RADIUS, FLAGS from {};".format(table_name)
                print "\nsubmitting query: "
                print "    ", query
                ...
                band_df = band_df.rename(index=str, columns={'ALPHAMODEL_J2000':'ALPHAMODEL_J2000_'+band,
                                                             ...
                                                             'FLUX_RADIUS':'FLUX_RADIUS_'+band,
                                                             'FLAGS':'FLAGS_'+band})
                band_dfs.append(band_df)

            elif table=='TRUTH':
                query = "select BALROG_INDEX, OBJTYPE, RA, DEC, ZEROPOINT, MAG, FLUX_0, FLUX_NOISELESS, FLUX_NOISED, NOT_DRAWN, HALFLIGHTRADIUS_0 from {};".format(table_name)
                band_df = connection.query_to_pandas(query)
