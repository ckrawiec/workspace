...
def main(args):
    ...
    output_file = args[3]
    debug_file = os.path.splitext(output_file)[0]+'_debug.fits'
    if len(args)==6:
    ...
    if debug==True:
        debug_ids = data_ids[:N_debug]
        print len(debug_ids), len(set(debug_ids))
        z_mask = (z >= np.min(z_debug)) & (z < np.max(z_debug))
        template_z = z[z_mask]
        template_ids = templates['COADD_OBJECTS_ID'][z_mask]

        template_zip = np.array( zip( *[templates[data_type+'_'+SE_col+'_'+f][z_mask] for f in filters] ) )

        col_defs = [fits.Column(name='template_z', format='D', array=template_z),
                    fits.Column(name='template_id', format='K', array=template_ids)]

        for n in range(N_debug):
            template_Ls = psingle(data_zip[n], err_zip[n], template_zip)

            col_defs.append(fits.Column(name=str(int(debug_ids[n])), format='D', array=template_Ls))

        pri_hdr = fits.Header()
        tb_hdr = fits.Header()
        tb_hdr['COMMENT'] = "Gaussian likelihoods for data in {} using photo-zs of templates from {}. \
        Data vectors/errors were comprised of {}(ERR)_{}_% for % in {}.".format(data_file, 
                                                                                template_file, 
                                                                                data_type, 
                                                                                SE_col, 
                                                                                filters)
        ...
        tb_hdu = fits.BinTableHDU.from_columns(fits.ColDefs(col_defs), nrows=len(template_zip), header=tb_hdr)
        hdu_list = fits.HDUList([pri_hdu, tb_hdu])
        hdu_list.writeto(debug_file, clobber=True)
